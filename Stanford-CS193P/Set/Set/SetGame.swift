...
struct SetGame {
    ...
    private(set) var cardsOnTable = [SetCard]() // карты на игровом поле в данный момент
    private(set) var cardsSelected = [SetCard]() // выбранные карты, не больше 3
    private(set) var cardsTryMatched = [SetCard]() // 3 карты для испытания на Set
    private(set) var cardsRemoved = [SetCard]() // карты составившие Set и удаленные из игры
    ...
    mutating func chooseCard(at index: Int) {
        assert(
            cardsOnTable.indices.contains(index),
        ...
        if !cardsRemoved.contains(cardChoosen) && !cardsTryMatched.contains(cardChoosen) {
            if isSet != nil {
                // на прошлом шаге была попытка получить Set
                if isSet! {
...
extension Array where Element: Equatable {
    // переключаем присутствие элемента в массива:
